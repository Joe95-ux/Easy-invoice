...
export function EstimateSendDialog({
  ...
  const [subject, setSubject] = useState(defaultSubject(estimateNumber, companyName));
  ...
  const canSend = !TERMINAL_STATUSES.includes(status);
  const attachmentName = `${estimateNumber}.pdf`;

  useEffect(() => {
    if (!open) return;
    setTo(clientEmail ?? "");
    setSubject(defaultSubject(estimateNumber, companyName));
    setMessage("");
  ...
  }, [open, clientEmail, estimateNumber, companyName]);
  ...
  async function handleAiDraft() {
    setDrafting(true);
    try {
      const response = await fetch(`/api/estimates/${estimateId}/email-draft`, {
        method: "POST",
  ...
  async function handleSend() {
    ...
    try {
      ...
      toast.success(`Estimate sent to ${to.trim()}`);
      onOpenChange(false);
      onSent?.();
      router.refresh();
    } catch (error) {
          ...
          <header className="flex items-center justify-between gap-3 border-b bg-muted/40 px-4 py-2.5">
            <div className="min-w-0">
              ...
              <SheetDescription className="truncate text-xs">
                Send {estimateNumber} with PDF attached
              </SheetDescription>
            </div>
              ...
              <div className="flex min-h-0 flex-1 flex-col">
                ...
                <ComposeField label="Subject">
                  <Input
                    ...
                    placeholder={defaultSubject(estimateNumber, companyName)}
                    disabled={!canSend}
                ...
                <div className="border-t px-4 py-3">
                  ...
                  {!canSend && (
                    <p className="mt-2 text-xs text-muted-foreground">
                      This estimate cannot be sent in its current status.
                    </p>
                  )}
                </div>
              </div>
