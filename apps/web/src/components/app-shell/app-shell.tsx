...
/** Full-screen tool UIs (form builder) — no app sidebar/header chrome. */
function isImmersivePath(pathname: string) {
  return /\/projects\/[^/]+\/forms\/[^/]+\/edit\/?$/.test(pathname);
}
        ...
        <CompanyPlanProvider plan={plan}>
          ...
          <div
            className={cn(
              ...
              !immersive && "md:mt-2 md:mr-2 md:peer-data-[state=collapsed]:ml-2",
            )}
          >
            <div
              className={cn(
          ...
          {!immersive ? <TimeTimerShell activeCompanyId={activeCompanyId} /> : null}
        </CompanyPlanProvider>
      </TimeTimerProvider>
    </SidebarProvider>
  );
